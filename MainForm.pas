...
begin
  if Checkbox1.Checked then
    UpdateCmb(ComboBox2,length(ints[ComboBox3.ItemIndex]),'Уровень')
  else
  begin
    if not ComboBox2.Enabled then ComboBox2.Enabled:=true;
    ComboBox2.Items.Clear;
    ComboBox2.Items.Add('Уровень 1');
  ...
  case Key of
    ...
    Chr(13): if (length(FactorsEdit.Text)=0) or (strtoint(FactorsEdit.Text)=0)
      then showmessage('Введите количество факторов (целое число > 0)')
        ...
        UpdateCmb(ComboBox1,length(ints),'Фактор');
        UpdateCmb(ComboBox3,length(ints),'Фактор');
